...
function Lobby({ onCreate, onJoin, initialCode }) {
  ...
  return (
    <div className={styles.lobby}>
      <div className={styles.logo}>url<span>chat</span></div>
      ...
      <div className={styles.card}>
        ...
        />
        <button className={`${styles.btn} ${styles.btnGhost}`} onClick={handleJoin}>
          Join
        </button>
      </div>
    </div>
...
function Waiting({ roomCode, onBack, onToast }) {
  ...
  return (
    <div className={styles.waiting}>
      ...
      <div className={styles.copyHint}>click code to copy · or share the link below</div>

      <div className={styles.urlShare} onClick={copyUrl} title="click to copy link">
        {shareUrl}
      </div>
...
function Chat({ roomCode, conn }) {
  const [messages, setMessages] = useState([
    { id: 0, type: 'sys', text: 'connected ✦  start typing — your colleague sees the url bar change live' }
  ])
  ...
  useEffect(() => {
    ...
    const onData = (data) => {
      if (data.type === 'typing') {
        ...
        setMessages(prev => [...prev, { id: msgIdRef.current++, type: 'theirs', text: data.text, time }])
      }
    }

    const onClose = () => {
      setDisconnected(true)
      setMessages(prev => [...prev, { id: msgIdRef.current++, type: 'sys', text: 'colleague disconnected' }])
      const params = new URLSearchParams()
      params.set('room', roomCode)
      window.history.replaceState(null, '', `?${params.toString()}`)
      document.title = 'urlchat'
    ...
    return () => { conn.off('data', onData); conn.off('close', onClose) }
  }, [conn, roomCode])
  ...
  function sendMessage() {
    ...
    const now = new Date()
    const time = now.getHours().toString().padStart(2,'0') + ':' + now.getMinutes().toString().padStart(2,'0')
    setMessages(prev => [...prev, { id: msgIdRef.current++, type: 'mine', text, time }])
    setInputVal('')
    pushToUrl('')
  }
    ...
    <div className={styles.chatWrap}>
      <div className={styles.chatHeader}>
        <div
          ...
          style={disconnected ? { background: 'var(--muted)', boxShadow: 'none' } : {}}
        />
        <div className={styles.chatTitle}>
          {disconnected ? 'disconnected' : peerTyping ? 'colleague is typing…' : 'connected'}
        </div>
      ...
      <div className={styles.messages}>
        {messages.map(msg => {
          if (msg.type === 'sys') return (
            <div key={msg.id} className={styles.sysMsg}>{msg.text}</div>
          )
          return (
            <div key={msg.id} className={`${styles.msg} ${msg.type === 'mine' ? styles.mine : styles.theirs}`}>
